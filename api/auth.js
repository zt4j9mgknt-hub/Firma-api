...
function SantiereTab({ santiere, setSantiere, rapoarte, setRapoarte, etape, setEtape, persist, company, materials, sculeSantier, setSculeSantier, cereriSantier, setCereriSantier, miscariSantier, setMiscariSantier, onExportToDeviz, onExportMaterialeFolosite, onCerereMaterial, currentUser }) {
  const [form, setForm] = useState(null);
  ...
  const addToEchipa = (santierId, userId) => {
    ...
    const echipa = s.echipa || [];
    if (!echipa.includes(userId)) updateEchipa(santierId, [...echipa, userId]);
  };
  const removeFromEchipa = (santierId, userId) => {
    const s = santiere.find((x)=>x.id===santierId);
    updateEchipa(santierId, (s.echipa || []).filter((id)=>id!==userId));
  };
    ...
    return (
      <div>
        ...
        {currentUser?.rol === 'Manager' && (
          <div className="rounded-xl p-3 mb-4" style={{ background:'#1B2127', border:'1px solid #2A323B' }}>
            <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex flex-wrap gap-1.5 mb-2">
              {(santier?.echipa || []).length === 0 && <span className="text-xs" style={{ color:'#5C6570' }}>Niciun electrician asignat.</span>}
              {(santier?.echipa || []).map((userId)=>{
                const u = allUsers.find((x)=>x.id===userId);
                return (
                  <span key={userId} className="inline-flex items-center gap-1 text-xs pl-2.5 pr-1 py-1 rounded-lg" style={{ color:'#E9E7E2', background:'#171C21' }}>
                    🔌 {u?.nume || '(cont șters)'}
                    <button onClick={()=>removeFromEchipa(santier.id, userId)} style={{ color:'#E05252', opacity:0.8 }}>✕</button>
                  </span>
                );
              })}
            </div>
        ...
        <div className="flex items-center gap-2 mb-4 flex-wrap">
          ...
          {rapoarte.filter((r)=>r.santierId===selected).some((r)=>(r.materialeFolosite||[]).length>0) && (
            <button onClick={()=>onExportMaterialeFolosite(rapoarte.filter((r)=>r.santierId===selected), santier?.nume)} className="text-sm px-3 py-1.5 rounded-lg font-medium" style={{ color:'#14181C', background:'#4CAF7D' }}>🧾 Exportă materiale folosite în Deviz</button>
          )}
        </div>

        {reportForm && <RaportForm initial={reportForm.id?reportForm:null} etape={etapeSantier} echipaPermisa={santier?.echipa || []} materials={materials} onCancel={()=>setReportForm(null)} onSave={saveReport} />}
...
function Dashboard({ company, devize, offers, invoices, santiere, materials, cereriSantier, rapoarte, setTab, currentUser }) {
  const roleIcon = ROLES.find((r)=>r.id===currentUser?.rol)?.icon || '👤';
  ...
  const santiereMele = santiere.filter((s)=>(s.echipa || []).includes(currentUser?.id));
          ...
          {mementoOpen && (
            <div className="space-y-2 mt-2">
              {santiereFaraRaportAzi.map((s)=>(
                <button key={s.id} onClick={()=>setTab('santiere')} className="w-full text-left rounded-xl px-4 py-3 flex items-center justify-between" style={{ background:'#1B2127', border:'1px solid #E0525266' }}>
                  <span className="text-sm" style={{ color:'#E9E7E2' }}>Nu ai trimis raportul de azi pentru „{s.nume}"</span>
        ...
        <div className="mb-6">
          ...
          {santiereOpen && (
            santiereMele.length === 0 ? (
              ...
              <div className="space-y-2 mt-2">
                {santiereMele.map((s)=>(
                  <button key={s.id} onClick={()=>setTab('santiere')} className="w-full text-left rounded-xl px-4 py-3 flex items-center justify-between" style={{ background:'#1B2127', border:'1px solid #2A323B' }}>
                    <span className="text-sm" style={{ color:'#E9E7E2' }}>{s.nume} <span className="text-xs ml-2" style={{ color: s.status==='Activ'?'#4CAF7D':'#8B93A0' }}>{s.status}</span></span>
      ...
      {(materialeSubStoc.length > 0 || cereriInAsteptare.length > 0) && (
        <div className="mb-6">
          ...
          <div className="space-y-2">
            ...
            {isManager && cereriInAsteptare.length > 0 && (
    ...
    <div className="min-h-screen w-full" style={{ background:'#14181C', fontFamily:"'Inter', sans-serif" }}>
      ...
      <main className="flex-1 p-3 md:p-6 max-w-4xl w-full min-w-0">
        {loading ? (
          ...
          <Dashboard company={company} devize={devize} offers={offers} invoices={invoices} santiere={santiere} materials={materials} cereriSantier={cereriSantier} rapoarte={rapoarte} setTab={setTab} currentUser={effectiveUser} />
        ) : (
          <>
